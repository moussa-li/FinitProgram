  ...
  <div class="appcontainer">
    ...
    <div class="flow-menu">
      ...
      <div class="menu-list">
        ...
        >
        {{title.title}}
        <div class="menu-item" v-for="item in title.menueList" draggable="true" @dragstart="drag(item)">
          <!-- <i :class="item.icon"></i> -->
          <div>{{item.name}}</div>
        </div>
        <hr>
      ...
      <div class="operation-list">
        <div class="operation-item" @click="isConnect=true">
          <i class="el-icon-bottom-right"></i>
          <div>连线</div>
        </div>
        <div class="operation-item" @click="isConnect=false">
    ...
    <div class="flow-detail">
      ...
      <div class="savebtn">
        <el-button type="success" size="mini" @click="runData()">运 行</el-button>
      </div>
    </div>
    <div class="visual">
      <div class="flow-title">展示栏</div>
      <view_3D>

      </view_3D>
    </div>
    <div class="flow-edit">
      <div class="flow-title">参数栏</div>
      <div class="flow-edit-content">
        <!-- <edit-flow ref="flowEdit" v-show="editType=='flow'"></edit-flow> -->
        <edit-node ref="nodeForm" :lineList="data.lineList" :nodeList="data.nodeList"></edit-node>
...
<script>
  ...
  import titleList from '@/store/Finite_Analyze/menueList'
  import initNode from '@/store/Finite_Analyze/initNode'
  // import top_nav from './nav.vue'
  import view_3D from './view.vue'
  // import $ from 'jquery';
  export default {
    name: 'flowMain',
    data() {
      return {
        ...
        data: {
          ...
          nodeList:initNode.nodeList,
          lineList:initNode.lineList,
        },
    ...
    methods: {
      ...
      //编辑节点
      editNode(nodeId) {
        // for(var i in this.data.lineList)
        // {
        //   console.log('line', this.data.lineList[i])
        // }
        this.editType = 'node';
        this.$nextTick(function() {
      ...
      runData() {
        ...
        this.$axios.post('http://127.0.0.1:5000/runscript',this.data.nodeList).then(res=>{
          alert(res.data)
